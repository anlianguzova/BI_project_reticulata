...
### Input data ###
expr_unaveraged <- read.csv2("/home/lisa/raki_outputs/new_expression_data.csv", 
                             header = T, sep=",", row.names = 1)
expr_averaged <- read.csv2("/home/lisa/raki_outputs/new_mean_expression_data.csv", 
                           header = T, sep=",", row.names = 1)
colnames(expr_unaveraged) <- c("Im_12", "If_11", 
                               "Hm_2", "If_3", "Im_4", 
                               "Hm_10", "Hf_1")
Healthy_male <- c(0, 0, 1, 1)
Infected_male <- c(1, 1, 0, 0)
Sample <- c("Im_4", "Im_12",  "Hm_2", "Hm_10")
design <- data.frame(Sample, Healthy_male, Infected_male)
...
expr_males <- expr_unaveraged %>% select(all_of(Sample)) 
expr_males <- mutate_all(expr_males, function(x) as.numeric(as.character(x)))
expr_males$GeneIDs <- as.factor(rownames(expr_males))
design_matrix <- data.matrix(design_matrix, rownames.force = T)

### Analysis ###
# compute statistics and p-values
RNresults <- RN_calc(expr_males, design = design_matrix)
...
write.table(RNresults_significant, 
            file=sprintf("raki_outputs/%s_RNentropy_significant_results.tsv", species_tag),
            sep="\t", col.names = T, row.names = F)

### Subset over-expressed genes only ###
colnames(RNresults_significant)

for (sample in c("Healthy_male", "Infected_male")){
  sample_overexpr <- subset(RNresults_significant, 
                            RNresults_significant[[sample]] == "1")
  write.table(sample_overexpr, file=sprintf("raki_outputs/%s_%s_RNentropy_overexp.tsv", 
...
write.table(RNresults_npmi_matrix, file=sprintf("raki_outputs/%s_RNentropy_npmi.tsv", 
                                                species_tag), sep="\t", 
