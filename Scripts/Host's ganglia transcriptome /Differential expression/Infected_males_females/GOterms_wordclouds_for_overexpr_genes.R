...
### Input files ###
fem_reduced <- read.csv2("Pmin_Infected_fem_overexpr_genes_reduced_GSEA_results.Dmelanogaster.tsv", 
                             header = T, sep="\t")
male_reduced <- read.csv2("Pmin_Infected_male_overexpr_genes_reduced_GSEA_results.Dmelanogaster.tsv", 
                              header = T, sep="\t")
species_tag <- "Pmin"
...
diff_terms <- setdiff(male_reduced$term, fem_reduced$term)
diff_reduced <- male_reduced[male_reduced$term %in% diff_terms, ]

### Processing ###
input_list <- list("infected_fem" = fem_reduced,
                   "infected_male" = male_reduced,
                   "infected_fem_vs_male" = diff_reduced)
